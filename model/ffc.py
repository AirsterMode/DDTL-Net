...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # (batch, c, h, w/2+1, 2)
        fft_dim = (-3, -2, -1) if False else (-2, -1)
        ...
        ffted = self.conv_layer(ffted) + ffted  # (batch, c*2, h, w/2+1)
        ...
        ifft_shape_slice = x.shape[-3:] if False else x.shape[-2:]
...
if __name__ == '__main__':
    temp = torch.randn(1, 64, 11, 11)
    net =FourierUnit(in_channels=64 ,out_channels=64)
    out = net(temp)
    print(out.shape)